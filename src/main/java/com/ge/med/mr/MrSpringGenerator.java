...
public class MrSpringGenerator extends AbstractJavaCodegen implements BeanValidationFeatures
{
    ...
    protected String serviceName = "petstore";
    protected String configPackage = "com.ge.med.mr.ws";
    protected String basePackage = "com.ge.med.mr";
    ...
    public MrSpringGenerator()
    {
        ...
        apiPackage = "com.ge.med.mr.api";
        modelPackage = "com.ge.med.mr.model";
        invokerPackage = "com.ge.med.mr.api";
    ...
    {
        return "Generates a Java SpringBoot Server application [mr-spring] using the SpringFox integration.";
    }
    ...
    public void processOpts()
    {
        ...
        supportingFiles.add(new SupportingFile("Imakefile.mustache", "", "Imakefile"));

        if (additionalProperties.containsKey(SERVICE_NAME))
        {
            serviceName = (String) additionalProperties.get(SERVICE_NAME);
        }
        supportingFiles.add(new SupportingFile("serviceSpec.mustache", "", serviceName + ".yaml"));

        //deploy related
        supportingFiles.add(new SupportingFile("serviceJar.mustache", "deploy", serviceName + "-boot.jar"));
        supportingFiles.add(new SupportingFile("init.mustache", "deploy", "MR" + serviceName + ".init"));
        supportingFiles.add(new SupportingFile("runServiceSh.mustache", "deploy", "run-" + serviceName + ".sh"));
        supportingFiles.add(new SupportingFile("toProduct.mustache", "deploy", "toProduct"));

        if (this.interfaceOnly && this.delegatePattern)
        {
            throw new IllegalArgumentException(
                    String.format("Can not generate code with `%s` and `%s` both true.", DELEGATE_PATTERN,
                                  INTERFACE_ONLY));
        }

        if (!this.interfaceOnly)
        {
            String expModule = serviceName + "-api" + File.separator;
            String implModule = serviceName + "-impl" + File.separator;
            String wsModule = serviceName + "-ws" + File.separator;
            if (library.equals(DEFAULT_LIBRARY))
            {
                supportingFiles.add(new SupportingFile("homeController.mustache",
                                                       (wsModule + sourceFolder + File.separator + configPackage)
                                                               .replace(".", File.separator),
                                                       "SwaggerController.java"));
                supportingFiles.add(new SupportingFile("swagger2SpringBoot.mustache",
                                                       (wsModule + sourceFolder + File.separator + basePackage)
                ...
                supportingFiles.add(new SupportingFile("application.mustache",
                                                       (wsModule + "src.main.resources").replace(".", File.separator),
    ...
    {
        return outputFolder + File.separator + serviceName + "-api" + File.separator + sourceFolder + File.separator
